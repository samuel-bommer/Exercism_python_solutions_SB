...
def round_scores(student_scores):
    ...
    for scores in student_scores:
        rounded_student_scores = round(scores)
        rounded_scores.append(rounded_student_scores) 
...
def letter_grades(highest):
    ...
    elements = 4
    passing = 40
    step_size = int((highest - passing) // elements)
    
    list_grades = []
    for element in range(elements): 
        grade = passing + 1 + element * step_size
...
def student_ranking(student_scores, student_names):
    ...
    ranked_list = []
    for std in range(len(student_scores)):
        element_student = f'{std+1}. {student_names[std]}: {student_scores[std]}'
        ranked_list.append(str(element_student))
