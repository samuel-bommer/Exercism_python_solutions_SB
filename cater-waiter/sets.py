...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    no_dups = set(dish_ingredients)
    return (dish_name, no_dups)
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for diet_name, diet_set in dict_diets.items():
        if set(dish_ingredients) <= diet_set:
            return f'{dish_name}: {diet_name}'
    return f'{dish_name} not in {dict_diets}'
...
def tag_special_ingredients(dish):
    ...
    if set(dish[1]) & SPECIAL_INGREDIENTS:
        intersec = set(dish[1]) & SPECIAL_INGREDIENTS
        return (dish[0], intersec)
    return (dish[0], str('no special ingredients'))
...
def separate_appetizers(dishes, appetizers):
    ...
    dish_diff_ap = set(dishes) - set(appetizers)
    return list(dish_diff_ap)
...
def singleton_ingredients(dishes, intersection):
    ...
    singletons = []
    for dish in dishes:
        singletons_per_dish = set(dish) - intersection
        single_list = list(singletons_per_dish)
        singletons.append(single_list)
    
    singletons_onelist = [item for sublist in singletons for item in sublist]
    
    set_singletons = set(singletons_onelist)
    return set_singletons
